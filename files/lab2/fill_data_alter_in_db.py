...
# Для загрузки данных через script - обязательно нужно прописать эти 2 строки,
# подгружающие настройки Django
...
if __name__ == "__main__":
    ...
    t1 = time()
    data_for_write = [Author(**data) for data in data_author]  # Здесь просто создались объекты,
    # записи в БД не было
    ...
    объектов в базу данных(Это сделано, чтобы разработчик мог сам выбрать, когда и на что 
    тратить ресурсы при работе с БД). 
    Однако возможно явно вызвать эти проверки и обработать возможные исключения, 
    чтобы убедиться, что данные соответствуют заданным
    ...
    for entry in data_entry:
        ...
        # pub_date в моделях объявлен как DateTimeField, поэтому на вход необходимо
        # подавать объект datetime
        pub_date = datetime(*map(int, re_split.split(entry["pub_date"]))) if \
            entry["pub_date"] is not None else datetime.now()
        pub_date = timezone.make_aware(pub_date)  # добавляем данных о часовом поясе,
        # так как могут быть проблемы  с БД и Django
        obj = Entry(blog=blog,
